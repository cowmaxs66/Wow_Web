...
function Write-PackageReadme {
    ...
    if ($PackageKind -eq 'server') {
        $content = @(
            ...
            '- Double-click `WoW-Desktop.exe` to open the desktop console directly. If Server is not running, it will start `bin/management-server-core.exe --no-open-browser` automatically.',
            ...
            '',
            '## Lua hot push',
            '- Open Web Admin, select one or more Clients, then use Remote Actions to push Lua scripts, start Lua, stop Lua, or query Lua status.',
            '',
            '## Package boundary',
    ...
    if ($PackageKind -eq 'client') {
        $content = @(
            ...
            '- This package defaults to internal test mode: Lua is enabled, script security is disabled, and known Lua permissions include host.log, config.read, and dm.access.',
            '',
    ...
    $content = @(
            ...
            '## Main entry points',
            '- Double-click `management-server.exe` to start the Server tray UI. The tray starts Server and opens `WoW-Desktop.exe`.',
            '- Double-click `WoW-Desktop.exe` to open the desktop console directly. If Server is not running, it will start `bin/management-server-core.exe --no-open-browser` automatically. It uses Windows WebView2 Runtime internally, but does not start Edge browser or `msedge.exe --app`.',
            '- Double-click `client-agent.exe` to start Client tray UI.',
            ...
            '- Web Admin Remote Actions can push Lua scripts to selected Clients, start Lua, stop Lua, and query Lua status.',
        '',
        '## Maintenance entry points',
        '- `bin/management-server-core.exe --no-open-browser`',
        '- `bin/management-server-core.exe --tray`',
